...
namespace TrtlBotSharp
{
    partial class TrtlBotSharp
    {
        ...
        public static async void RunBotAsync()
        {
            ...
            // Load local files
            Log(0, "ETRXCurrencyService", "Loading config");
            await LoadConfig();
            Log(0, "ETRXCurrencyService", "Loading database");
            await LoadDatabase();
            ...
            Log(0, "ETRXCurrencyService", "Starting discord client");
            await RegisterCommandsAsync();
            await _client.LoginAsync(TokenType.Bot, botToken);
            await _client.StartAsync();

            // Set tip bot address
            Log(0, "ETRXCurrencyService", "Setting default address");
            await SetAddress();
        ...
        private static Task Ready()
        {
            ...
            if (!Monitoring)
            {
                Log(0, "ETRXCurrencyService", "Starting wallet monitor");
                BeginMonitoring();
                Monitoring = true;
            }
        ...
        private static Task Log(LogMessage arg)
        {
            ...
            if (arg.Message.Contains("Disconnected"))
                Log(0, "ETRXCurrencyService", "Restarting bot...");
        ...
        private static async Task ReactionAddedAsync(Cacheable<IUserMessage, ulong> CacheableMessage, ISocketMessageChannel Channel, SocketReaction Reaction)
        {
            ...
            if (Emote == tipJoinReact)
            {
                ...
                {
                    await Reaction.User.Value.SendMessageAsync(string.Format("Your balance is too low! Amount + Fee = **{0:n8}** {1}",
                ...
                if (!CheckUserExists(Message.Author.Id))
                {
                    ...
                    try
                    {
                        ...
                        Response.Description = string.Format("Register your wallet with `{0}registerwallet <your {1} address>` " +
                            "to get started!\nTo create a wallet head to https://traaittchain.com/tools.html\n or download traaittPlatform Enterprise wallet!",
